...
AMLine = {cpoints={}, bbox={}, a=0, b=0, c=0}
...
function AMCub:intTest(x,y)
    ...
    if bbtest_status == 1.0 then 
        ...
        --Criar função anonima e fazer bisseção
        local f_bezier = function(t) x_t, y_t = evalBezier(3, 0, self.cpoints, t); return y_t end
        t_root = bissection(f_bezier, y, 0, 1, MAX_ITR, eps)
...
--Quadrática
AQuad = {cpoints={}, w=0, segments={}}

function AQuad:new(x0, y0, x1, y1, w, x2, y2)

    self.__index = self

    local cpoints = {x0, y0, x1/w, y1/w, x2, y2}
...
function ACub:new(x0, y0, x1, y1, x2, y2, x3, y3)
    ...
    for idx = 2,#t_values do
        ...
        --Calcular os valores de cada ponto de controle
        local x0,y0 = reparam_bezier(3, 0, 0, t_values[idx-1], t_values[idx], cpoints)
        local x1,y1 = reparam_bezier(3, 0, 1, t_values[idx-1], t_values[idx], cpoints)
        local x2,y2 = reparam_bezier(3, 0, 2, t_values[idx-1], t_values[idx], cpoints)
        local x3,y3 = reparam_bezier(3, 0, 3, t_values[idx-1], t_values[idx], cpoints)
