...
function _M.render(scene, window, viewport, file, args)
    ...
    scene:get_scene_data():iterate({
        painted_shape = function(self, winding_rule, shape, paint)
            ...
            local what = projectivity(2.0, 3.0, 5.0, -1.0, 12.0, 4.0, 0.0, 0.0, 1.0)
            pdata:iterate(filter.make_input_path_f_xform(what, {

                begin_contour = function(self, x0, y0) print("begin", x0, y0) end,
                linear_segment = function(self, x1, y1, x2, y2) print('line', x1, y1, x2, y2) end,
            }))
