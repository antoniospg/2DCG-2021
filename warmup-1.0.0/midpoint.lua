...
local function set_pixelxy(img, x, y)
...
function linex(img, x1, y1, x2, y2, set_pixel)
...
function line(img, x1, y1, x2, y2)
    ...
    if dx > dy then
        if x2 > x1 then
            linex(img, x1, y1, x2, y2, set_pixelxy)
        else
            linex(img, x2, y2, x1, y1, set_pixelxy)
        end
    else
        if y2 > y1 then
            linex(img, y1, x1, y2, x2, set_pixelyx)
        else
            linex(img, y2, x2, y1, x1, set_pixelyx)
        end
    end
end
