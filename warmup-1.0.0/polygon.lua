...
local function hline(img, y, x1, x2)
    assert(x2 >= x1)
    if x2 < 1 or x1 > img:get_width() then return end
    x1 = min(max(x1, 1), img:get_width())
    x2 = min(max(x2, 1), img:get_width())
    if y >= 1 and y <= img:get_height() then
        for x = x1, x2 do
            img:set_pixel(x, y, 1)
        end
    end
...
local xs, ys = star(5, 3, width, height)
--local vertices = 30
